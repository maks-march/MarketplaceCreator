          ...
          {auth.isAuthenticated && (
            <>
              <div
                ...
                role="button"
                tabIndex={0}
                onClick={() => auth?.isAuthenticated ? navigate(`/${auth?.role}/profile`) : navigate('/login')}
                onKeyDown={(e) => { if (e.key === 'Enter') (auth?.isAuthenticated ? navigate(`/${auth?.role}/profile`) : navigate('/login')); }}
